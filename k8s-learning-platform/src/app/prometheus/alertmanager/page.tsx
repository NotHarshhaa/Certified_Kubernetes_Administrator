...
export default function AlertmanagerPage() {
  return (
    <div className="space-y-8 md:space-y-12">
      ...
      <div>
        <h2 className="text-xl md:text-2xl font-bold text-foreground mb-4 md:mb-6 text-center">Notification Channels</h2>
                ...
                <div className="space-y-2">
                  {Object.entries(channel.configuration).map(([key, value]) => (
                    <div key={key} className="flex text-sm">
                      <span className="font-medium text-foreground w-24 md:w-32">{key}:</span>
      ...
      {/* Alert Routing */}
      <div>
        <h2 className="text-xl md:text-2xl font-bold text-foreground mb-4 md:mb-6">Alert Routing Configuration</h2>
        <Card>
          <CardHeader>
              ...
              {alertRouting.map((route, index) => (
                <div key={index} className="border rounded-lg p-4">
                  <div className="flex items-center justify-between mb-3">
                    <h4 className="font-medium text-foreground text-sm md:text-base">{route.name}</h4>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 text-sm">
                    ...
                    <div>
                      ...
                      <span className="ml-1 text-muted-foreground">{route.groupBy.join(', ')}</span>
                    </div>
                    <div>
                      <span className="font-medium">Group Wait:</span>
                      <span className="ml-1 text-muted-foreground">{route.groupWait}</span>
                    </div>
                    <div>
                      <span className="font-medium">Group Interval:</span>
                      <span className="ml-1 text-muted-foreground">{route.groupInterval}</span>
                    </div>
                    <div>
                      <span className="font-medium">Repeat Interval:</span>
                      <span className="ml-1 text-muted-foreground">{route.repeatInterval}</span>
                    </div>
                  </div>
                </div>
              ))}
      ...
      {/* Key Features */}
      <div>
        <h2 className="text-xl md:text-2xl font-bold text-foreground mb-4 md:mb-6">Key Features</h2>
          ...
          {features.map((feature, index) => (
            <Card key={index} className="text-center hover:shadow-lg transition-shadow">
              <CardContent className="pt-4 md:pt-6">
                ...
                <p className="text-xs md:text-sm text-muted-foreground mb-2">{feature.description}</p>
      ...
      <div>
        <h2 className="text-xl md:text-2xl font-bold text-foreground mb-4 md:mb-6">Sample Configuration</h2>
        <Card>
          <CardHeader>
...
- name: 'warning-team'
  ...
  - api_url: 'https://hooks.slack.com/services/...'
    ...
    text: '{{ range .Alerts }}{{ .Annotations.summary }}{{ end }}'`}
      ...
      {/* Access Alertmanager */}
      <div>
        <h2 className="text-xl md:text-2xl font-bold text-foreground mb-4 md:mb-6">Access Alertmanager</h2>
        <Card>
          <CardHeader>
          ...
          <CardContent>
            <div className="space-y-4">
              <div>
                <h4 className="font-medium text-foreground mb-2 text-sm md:text-base">1. Port Forward to Alertmanager</h4>
              ...
              <div>
                <h4 className="font-medium text-foreground mb-2 text-sm md:text-base">2. Access Alertmanager UI</h4>
              ...
              <div>
                <h4 className="font-medium text-foreground mb-2 text-sm md:text-base">3. Key Features Available</h4>
      ...
      <div className="bg-muted/50 rounded-2xl p-6 md:p-8 text-center">
        <h2 className="text-xl md:text-2xl font-bold text-foreground mb-4">Ready to Monitor External Apps?</h2>
